<?if (!defined("B_PROLOG_INCLUDED") || B_PROLOG_INCLUDED !== true) die();?><?if (!CModule::IncludeModule('intec.startshop')) return;?>
...
<div class="startshop-catalog<?=$arFlags['ADAPTABLE'] ? ' adaptiv' : ''?>" id="<?=$sUniqueID?>">
	...
	<?reset($arResult['MORE_PHOTO']);?>
		<div class="startshop-row">
			...
			<div class="startshop-information">
				<?if (!empty($arResult['PROPERTIES']['CML2_BRAND']['VALUE'])):?>
					<?$brand = $arResult['PROPERTIES']['CML2_BRAND']['VALUE']?>
		...
		<?elseif($options['CATALOG_PRODUCT_VIEW']['ACTIVE_VALUE'] == 'WITHOUT_TABS'):?>
		<div class="startshop-row">
			<?include('parts/ViewWithoutTabs.php')?>
		</div>
		<?elseif($options['CATALOG_PRODUCT_VIEW']['ACTIVE_VALUE'] == 'WITH_TABS_UP'):?>
		...
		<div id="expandables" class="startshop-item_description">
			...
			<?$APPLICATION->IncludeComponent(
				...
				Array(
					...
					"LINE_ELEMENT_COUNT" => ($arFlags['LEFT_MENU'])?'3':'4',
					...
					"CURRENCY" => ($arParams["USE_COMMON_CURRENCY"] == "Y") ? $arParams["CURRENCY"] : '',
					"USE_PRICE_COUNT" => "N",
					"SHOW_PRICE_COUNT" => "1",
					"PRICE_VAT_INCLUDE" => "Y",
