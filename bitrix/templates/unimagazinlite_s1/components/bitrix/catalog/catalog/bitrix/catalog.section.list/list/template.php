...
<div class="startshop-catalog<?=$arParams['ADAPTABLE'] == "Y" ? " adaptiv" : ""?>">
	...
	<?
		foreach ($arResult['SECTIONS'] as &$arSection)
		{
			if ($arSection['DEPTH_LEVEL'] == 1 || !($arParams['ROOT_SECTIONS'] == "Y")) 
			{
				...
				{
					$picture = CFile::ResizeImageGet($arSection['PICTURE']['ID'], array('width' => 200, 'height' => 200, BX_RESIZE_IMAGE_EXACT));
				}
				...
				?><div class="startshop-element startshop_col startshop-50" id="<?=$this->GetEditAreaId($arSection['ID']);?>">
					<div class="startshop-content">
						<div>
							<a class="startshop-image" href="<?=$arSection["SECTION_PAGE_URL"]; ?>">
								<div class="startshop-aligner-vertical"></div>
								<img src="<?=$picture['src']?>" alt="<?=htmlspecialcharsbx($arSection["NAME"])?>" title="<?=htmlspecialcharsbx($arSection["NAME"])?>"/>
							</a>
	...
	?>
	<?$frame->end()?>
		<div class="description startshop_col startshop-100"><?=$arResult['IBLOCK']['DESCRIPTION']?></div>
	</div>
</div>
