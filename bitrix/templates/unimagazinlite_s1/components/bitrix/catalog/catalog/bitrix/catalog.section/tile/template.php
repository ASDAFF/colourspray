...
<div class="startshop-catalog<?=$arParams['ADAPTABLE'] == "Y" ? " adaptiv" : ""?>">
	...
	<?if (!empty($arResult['ITEMS'])):?>
		...
		<?  $url_parent_group= $arResult["SECTION_PAGE_URL"] ?>
		<?  $name_parent_group= $arResult['NAME']?>
			...
			<?foreach($arResult["ITEMS"] as $sKey => $arItem):?>
                ...
                <div class="startshop_col <?=$sGridStyle?>">
                    <div class="startshop-element startshop-hover-shadow" id="<?=$this->GetEditAreaId($arItem['ID']);?>">
						...
						<a href="<?=$arItem["DETAIL_PAGE_URL"]?>" class="image_product" style="background-image:url(<?=$src?>)" title="<?=htmlspecialcharsbx($arItem['NAME'])?>">	
							<div class="marks">
                                ...
                                <?if ($arItem['STARTSHOP']['AVAILABLE']) {?>
                                    <div class="startshop-buy">
										<?//if (!$arItem['STARTSHOP']['BASKET']['INSIDE']):?>
                                           <a rel="nofollow" class="startshop-button startshop-button-standart to-cart addToBasket addToBasket<?=$arItem["ID"]?>" id="to_cart_<?=$arItem['ID']?>" onclick="addToBasket('<?=SITE_DIR?>', '<?=$arParams["IBLOCK_TYPE"]?>', '<?=$arParams["IBLOCK_ID"]?>', 'Add', '<?=$arItem["ID"]?>', 1, '<?=$arResult['STARTSHOP']['PRICES']['MINIMAL']['TYPE']?>','<?=$arParams["COMPARE_NAME"]?>','<?=$arParams["CFO_USE_FASTORDER"]?>',
												'<?=$arParams["OCB_PROP_NAME"]?>',
												'<?=$arParams["OCB_PROP_PHONE"]?>',
												'<?=$arParams["OCB_PROP_COMMENT"]?>')">
													<?=GetMessage("SH_CS_TILE_ADD_TO_BASKET")?>
											</a>
											<a rel="nofollow" href="<?=$arParams["BASKET_URL"];?>" id="in_cart_<?=$arItem['ID']?>" class="startshop-button startshop-button-standart startshop-status-focus to-cart-added addedToBasket addedToBasket<?=$arItem["ID"]?>" style="display: none;">
