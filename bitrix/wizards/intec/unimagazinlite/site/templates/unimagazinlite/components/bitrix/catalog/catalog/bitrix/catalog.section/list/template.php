				...
				<div class="startshop-element startshop-hover-shadow" id="<?=$this->GetEditAreaId($arItem['ID']);?>">
					...
					<a class="startshop-image-wrapper" href="<?=$arItem["DETAIL_PAGE_URL"]?>">
						<div class="startshop-image">
							...
							<img src="<?=$sImage?>" alt="<?=htmlspecialcharsbx($arItem['NAME'])?>" title="<?=htmlspecialcharsbx($arItem['NAME'])?>" />
						</div>
					</a>
								...
								<?if (empty($arItem['STARTSHOP']['OFFERS'])):?>
									...
									<?if ($arItem['STARTSHOP']['AVAILABLE']) {?>
										<div class="startshop-buy">
											...
											<?//else:?>
												<a rel="nofollow" href="<?=$arParams["BASKET_URL"];?>" id="in_cart_<?=$arItem['ID']?>" class="startshop-button startshop-button-standart startshop-status-focus to-cart-added addedToBasket addedToBasket<?=$arItem["ID"]?>">
