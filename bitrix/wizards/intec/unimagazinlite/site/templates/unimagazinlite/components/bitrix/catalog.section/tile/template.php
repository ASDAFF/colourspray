                                ...
                                <?if ($arItem['STARTSHOP']['AVAILABLE']) {?>
                                    <div class="startshop-buy">
                                        <?//if (!$arItem['STARTSHOP']['BASKET']['INSIDE']):?>
                                            <a rel="nofollow" class="startshop-button startshop-button-standart to-cart addToBasket addToBasket<?=$arItem["ID"]?>" id="to_cart_<?=$arItem['ID']?>" onclick="addToBasket('<?=SITE_DIR?>', '<?=$arParams["IBLOCK_TYPE"]?>', '<?=$arParams["IBLOCK_ID"]?>', 'Add', '<?=$arItem["ID"]?>', 1, '<?=$arResult['STARTSHOP']['PRICES']['MINIMAL']['TYPE']?>','<?=$arParams["COMPARE_NAME"]?>')">
